...
class Scrollmap {
  ...
  render() {
    ...
    if (!this.editor.component.refs.gutterContainer) {
      ...
      let markerHeight = Math.ceil(lineHeight/editorHeight*containerHeight);
      this.editor.element.style.setProperty('--scrollmap-marker-height', `${markerHeight}px`);
    }
    const main = atom.packages.getActivePackage("scrollmap")?.mainModule;
    const items = [];
    for (let [name, layer] of Object.entries(this.layers)) {
      if (main?.isLayerDisabled(name)) { continue; }
