...
module.exports = {
  ...
  /**
   * Toggles the linter layer and returns the linter UI interface.
  ...
  linterLayer() {
    ...
    return {
      ...
      render(args) {
        atom.workspace
          .getTextEditors()
          .map((editor) => editor.scrollmap.layers["linter"].filter(args));
      },
      didBeginLinting() {},
      didFinishLinting() {},
      dispose() {},
   ...
   * @returns {Object} Service with registerLayer and unregisterLayer methods
   */
  serviceProvider() {
    return {
      registerLayer: (name, Layer) => {
...
class Layer {
  ...
  prepareItems() {
    ...
    for (let item of this.items) {
      ...
      item.o = {
        click: () =>
          this.editor.scrollToScreenPosition([item.row, 0], { center: true }),
      };
    }
  }
