...
module.exports = {
  ...
  activate() {
    ...
    this.disposables.add(
      ...
      atom.commands.add("atom-workspace", {
        "scrollmap:toggle": () => this.showToggleView(),
      })
    );
  },
  ...
  showToggleView() {
    if (!this.toggleView) {
      ...
      this.disposables.add(
        this.toggleView,
        this.toggleView.onDidToggle(() => this.updateAllEditors())
      );
    }
  ...
  isLayerDisabled(name) {
    return this.disabledLayers.includes(name);
  },
  ...
  consumeScrollmap(provider) {
    ...
    if (provider.description) {
      this.descriptions[provider.name] = provider.description;
    }
    return new Disposable(() => {
      this.unregisterLayer(provider.name);
      delete this.descriptions[provider.name];
    });
  },
