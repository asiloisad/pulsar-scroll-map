...
class Layer {
  ...
  constructor(props) {
    ...
    this.items = []
    this.update = throttle(this.updateSync.bind(this), props.timer)
  }
  ...
  prepareItems() {
    ...
    for (let item of this.items) {
      ...
      let c = `scroll-item ${this.name}-layer`
      let s = `top:${this.editor.component.pixelPositionForScreenPosition({
        row: item.row, column:0 }).top/editorHeight*100}%`
      let o = { click: () => this.editor.scrollToScreenPosition([item.row, 0], { center:true }) }
      item.el = <div class={c} style={s} on={o}/>
    }
  }
...
class CursorLayer extends Layer {
    ...
    constructor(editor) {
      ...
      this.disposables.add(
        atom.config.observe('scroll-map.cursorLayer.threshold', (value) => {
...
class FindLayer extends Layer {
  ...
  constructor(editor) {
    ...
    this.disposables.add(
      atom.config.observe('scroll-map.findLayer.threshold', (value) => {
        this.threshold = value
      }),
      this.package.mainModule.findModel.onDidUpdate(() => {
        this.update()
      })
    )
  }
...
class NaviLayer extends Layer {
  ...
  constructor(editor) {
    ...
    this.disposables.add(
      atom.config.observe('scroll-map.naviLayer.threshold', (value) => {
        this.threshold = value
      }),
