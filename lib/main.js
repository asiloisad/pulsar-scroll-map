...
module.exports = {
  ...
  consumeFindService(service) {
    ...
    if (findModel?.onDidUpdate) {
      subscription = findModel.onDidUpdate(updateFindLayers);
    }
  ...
  consumeNaviService(naviService) {
    ...
    this.naviLayer();
    const updateNaviLayers = () => {
      atom.workspace.getTextEditors().forEach((editor) => {
        const layer = editor.scrollmap?.layers["navi"];
        if (layer) {
          layer.setData({
            editor: naviService.getEditor(),
            headers: naviService.getFlattenHeaders(),
          });
        }
      });
    };
    ...
    if (naviService.observeHeaders) {
      subscription = naviService.observeHeaders(updateNaviLayers);
    }
    return new Disposable(() => {
