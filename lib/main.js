...
module.exports = {
  ...
  setObservers() {
    this.disposables.add(
      ...
      atom.config.observe("scroll-map.diffLayer.state",
        () => this.diffLayer()),
      atom.config.observe("scroll-map.gitLayer.state",
        () => this.gitLayer()),
  ...
  gitLayer() {
    if (atom.config.get("scroll-map.gitLayer.state")) {
      const GitLayer = require("./layers/git");
      this.registerLayer("git", GitLayer);
    } else {
      this.unregisterLayer("git");
    }
  },
