...
class Layer {
  ...
  constructor(props) {
    ...
    this.configPath = props.configPath || `scrollmap-${this.name}`;
    this.items = [];
    this.clickHandlers = new Map();
    this.baseClass = `scrollmap-item scrollmap-${this.name}`;
    this.update = throttle(() => this.updateSync(), props.timer);
    this.disposables = new CompositeDisposable();
    this.disposables.add(
      atom.config.observe(`${this.configPath}.threshold`, (value) => {
        this.threshold = value;
      }),
      atom.config.onDidChange(`${this.configPath}.threshold`, () => {
        this.update()
      }),
